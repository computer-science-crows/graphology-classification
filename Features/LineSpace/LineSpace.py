...
def LineSpaceFeature(img):
    ...
    for i in range(len(delta)):
        if delta[i]>0:
            ...
            sub_img = img[0:img.shape[0],i*proportion:min(i*proportion + proportion, img.shape[1])]
            Spr = list(SPR(sub_img))
            
            peaks_max, _ = scy.find_peaks(Spr, distance = img.shape[1]//10)
            peaks_min, _ = scy.find_peaks([Spr[i] * -1 for i in range(len(Spr))], distance = img.shape[1]//10) 
            ...
            if len(peaks) == 1:
                ...
                total +=1 
            # plt.pyplot.plot(Spr[1:len(Spr)-1], range(0,img.shape[0]), color='red')
            # plt.pyplot.scatter([Spr[i] for i in peaks], peaks,[20]*len(peaks))
            # plt.pyplot.show()
    if width/(total+1) >=0.5:
